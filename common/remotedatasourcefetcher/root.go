...
type Provider interface {
	// UpdateRemoteDataSource Update the component specific data from a remote data source
	UpdateRemoteDataSource(ctx context.Context, name string, source RemoteDataSource) (int, error)
}
...
func New[T interface{}](r *reporter.Reporter, provider Provider, dataType string, dataSources map[string]RemoteDataSource) (*Component[T], error) {
	c := Component[T]{
		r:                r,
		provider:         provider,
		dataType:         dataType,
	...
	for name, source := range c.dataSources {
		...
		c.t.Go(func() error {
			...
			for {
				...
				count, err := c.provider.UpdateRemoteDataSource(ctx, name, source)
				cancel()
				if err == nil {
